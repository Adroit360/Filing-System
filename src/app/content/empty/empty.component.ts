...
export class EmptyComponent implements OnInit {
  ...
  constructor(
    ...
    private resourceManager: SharedResourceService,
    private activatedRoute: ActivatedRoute,
    private router: Router,
    private directory: DirectoryService,
    private approve: ApprovalService,
    ...
    this.resources = this.resourceManager.getMyResources(
      this.currentUser,
      data.getEntity()
    );
  ...
  computeRoute() {
    ...
    this.dirContent = this.directory.getSubDirectoryContent(
    ...
    // .subscribe(result => this.loaderService.setHttpProgressStatus(false));
  ...
  onFolderClicked(directory) {
    ...
    if (directory.itemType == "folder") {
      ...
      this.directory.recentFolders(
        this.data.getActiveUser().email,
        directory,
        this.data.getEntity(),
        this.data.getFirstRecentFolder(),
        this.data.getCurrentRecentFolderLength()
      );
  ...
  onModalDelete(result: boolean) {
    if (result) {
      if ((this.Receivedata.itemType = "file")) {
        console.log(result);
        this.directory.deleteFile(
          this.Receivedata.id,
          this.Receivedata.alias,
          this.data.getEntity()
        );
  ...
  sendFileToResource(item, resource) {
    ...
    this.resourceManager.AddFileToResource(
      item,
      resource,
      this.data.getEntity()
    );
  ...
  async Back() {
    ...
    let parent = await this.directory.getParent(
